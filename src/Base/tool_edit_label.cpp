...
void ToolEditLabel::setInfo(vector<QString> rendernamelist, int classification) {
	...
	auto allRenderList = this->glWidget->getProgressiveRenderMap();
	for (int i = 0; i < rendernamelist.size(); i++) {
		std::map<QString, ProgressiveRender*>::iterator cloud_it = allRenderList.find(rendernamelist[i]);
		if (cloud_it != allRenderList.end()) {
			renderlist.emplace_back(cloud_it->second);
		}
...
void EditLabelCommand::undo() {
	for (int i = 0; i < this->editInfoList.size(); i++) {
		...
		vector<int> selectBuffer = editInfo->undoSelectBuffer;
...
void EditLabelCommand::redo() {
	for (int i = 0; i < this->editInfoList.size(); i++) {
		auto editInfo = this->editInfoList[i];
		vector<int> selectBuffer;
		if (editInfo->redoSelectBuffer.empty()) {
			selectBuffer = editInfo->proRender->selectPointsByPolygon(glWidget->toolDrawPolygon->polygon, 0);
