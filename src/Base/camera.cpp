...
// opengl 3d坐标转屏幕坐标
Eigen::Vector2f Camera::convert_3dTo2d(const Eigen::Vector3f& world_point) {
	...
	window_cord[0] = (window_cord[0] + 1.0) / 2.0 * (float)size.x();
	window_cord[1] = -(window_cord[1] - 1.0) / 2.0 * (float)size.y();
