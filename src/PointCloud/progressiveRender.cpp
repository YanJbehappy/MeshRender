...
Eigen::MatrixXf ProgressiveRender::getColorStrip()
{
	...
	{
	case COLOR_COOL:
		return glWidget->getCoolRenderColor();
	case COLOR_GRAY:
		return glWidget->getGrayRenderColor();
	case COLOR_HOT:
		return glWidget->getHotRenderColor();
	case COLOR_JET:
		return glWidget->getJetRenderColor();
	default:
		break;
	}
}
...
// 根据Polygon区域选择点云
// polygon为屏幕坐标
vector<int> ProgressiveRender::selectPointsByPolygon(vector<Point> polygon, int SelectMode) {
